...
library(readxl)
data <- read_excel("whitewines.xls")


final<-data[sample(nrow(data), 1500), ]
...
#Part E)
#Linear regression cannot plot data well that is not inherently linear
```
...
####
values<-E$values
sum(values[1])/sum(values)

#BIPLOT
library(ggfortify)
autoplot(pca, data = final, loadings = TRUE, loadings.label=TRUE)
#is there clustering with respect to Species?
autoplot(pca, data = final, loadings = TRUE, loadings.label=TRUE, colour='quality')

biplot(pca)

autoplot(pca, data = final, loadings = TRUE, loadings.label=TRUE, colour='quality', frame=TRUE, frame.type='norm')
